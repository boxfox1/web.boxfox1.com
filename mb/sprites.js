...
if(sprite.constructor.name!=Uint8ArrayName){sprite=findSpriteInLibrary(thing,sprite,classes);}
...
return parsed;}
function expandObtainedSpriteMultiple(sprites,thing,width,height){var parsed={},sprite,part;thing.num_sprites=0;for(part in sprites){if((sprite=sprites[part]).constructor.name==Uint8ArrayName){++thing.num_sprites;parsed[part]=expandObtainedSprite(sprite,thing,width,height,true);}
...
if(nogood){if(check=current.normal){nogood=false;switch(check.constructor.name){case Uint8ArrayName:case"SpriteMultiple":return check;case"Object":current=check;break;default:current=current[check];break;}}else nogood=true;}
if(!nogood&&current){switch(current.constructor.name){case Uint8ArrayName:case"SpriteMultiple":return current;case"Object":continue;}}else{console.log("\nSprite not found! Title: "+ thing.title);console.log("Classname:",thing.className);console.log("Remaining",classes);console.log("Current",current);console.log("Prev",prev);return new Uint8ClampedArray(thing.spritewidth*thing.spriteheight);}}}
