...
function fastforward(num){pause();function resume(){for(i=speed=(num||1)- 1;i>0;--i)
step();unpause();}
...
if(two.up&&one!=two.up)return characterTouchesUp(one,two);if(two.solid||one.mario)
...
function objectOnTop(one,two){if(one.type=="solid"&&two.yvel>0)return false;if(one.yvel<two.yvel&&two.type!="solid")return false;if(one.mario&&one.bottom<two.bottom&&two.group=="enemy")return true;return((one.left+ unitsize<two.right&&one.right- unitsize>two.left)&&(one.bottom- two.yvel<=two.top+ two.toly||one.bottom<=two.top+ two.toly+ abs(one.yvel- two.yvel)));}
...
function characterTouchedSolid(me,solid){if(solid.up==me)return;if(characterOnSolid(me,solid)){if(solid.hidden)return;me.resting=solid;if(me.mario&&map.underwater)removeClass(me,"paddling");}
else if(solidOnCharacter(solid,me)){var mid=me.left+ me.width*unitsize/2;if(mid>solid.left&&mid<solid.right)me.undermid=solid;else if(solid.hidden)return;if(!me.under)me.under=[solid];else me.under.push(solid);if(me.mario){setTop(me,solid.bottom- me.toly+ solid.yvel,true);}
me.yvel=solid.yvel;if(me.mario)me.keys.jump=0;}
...
if(!me.mario){me.moveleft=!me.moveleft;if(me.group=="item")me.collide(solid,me);}
...
function scoreMarioShell(mario,shell){if(mario.star)return score(shell,200,true);if(!shell.resting)return score(shell,8000,true);if(shell.peeking)return score(shell,1000,true);return score(shell,100,true);}
...
scoreEnemyFin(enemy,amount);play("Kick");}
...
function moveFalling(me){if(me!=mario.resting)return me.yvel=0;shiftVert(me,me.yvel+=unitsized8);setBottom(mario,me.top);if(me.yvel>=unitsize*2.8){me.freefall=true;me.movement=moveFreeFalling;}}
function moveFallingScale(me){if(mario.resting==me){shiftScaleStringVert(me,me.string,me.yvel+=unitsized16);shiftScaleStringVert(me.partner,me.partner.string,-me.yvel);me.tension+=me.yvel;me.partner.tension-=me.yvel;}
...
function killNormal(me){if(!me)return;me.hidden=me.dead=true;me.alive=me.resting=me.movement=false;clearAllCycles(me);}
function killFlip(me,extra){flipVert(me);me.bottomBump=function(){};me.nocollide=me.dead=true;me.resting=me.movement=me.speed=me.xvel=me.nofall=false;me.yvel=-unitsize;addEvent(function(me){killNormal(me);},70+(extra||0));}
...
function emergeUp(me,solid){play("Powerup Appears");flipHoriz(me);me.nomove=me.nocollide=me.alive=me.nofall=me.emerging=true;determineThingQuadrants(me);switchContainers(me,characters,scenery);var move=setInterval(function(){shiftVert(me,-unitsized8);if(me.bottom<=solid.top){clearInterval(move);switchContainers(me,scenery,characters);me.nocollide=me.nomove=me.moveleft=me.nofall=me.emerging=false;if(me.emergeOut)me.emergeOut(me,solid);if(me.movement){me.movementsave=me.movement;me.movement=moveSimple;me.moving=addEventInterval(function(me,solid){if(me.resting!=solid){addEvent(function(me){me.movement=me.movementsave;},1,me);return true;}},1,Infinity,me,solid);}}},timer);}
function flicker(me,cleartime,interval){var cleartime=round(cleartime)||21,interval=round(interval)||3;me.flickering=true;addEventInterval(function(me){me.hidden=!me.hidden;},interval,cleartime,me);addEvent(function(me){me.flickering=me.hidden=false;},cleartime*interval+ 1,me);}
function killOtherCharacters(){var thing,i;for(i=characters.length- 1;i>=0;--i){thing=characters[i];if(!thing.nokillend)deleteThing(thing,characters,i);else if(thing.killonend)thing.killonend(thing);}
for(i=solids.length- 1;i>=0;--i)
if(solids[i].killonend)
deleteThing(solids[i],solids,i);}
function lookTowardMario(me,big){if(mario.right<=me.left){if(!me.lookleft||big){me.lookleft=true;me.moveleft=false;unflipHoriz(me);}}
else if(mario.left>=me.right){if(me.lookleft||big){me.lookleft=false;me.moveleft=true;flipHoriz(me);}}}
